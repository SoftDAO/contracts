 ...
 */
abstract contract PerAddressDistributor is IPerAddressDistributor, ReentrancyGuard {
  ...
  function getDistributionRecord(
    address beneficiary
  ) external view virtual returns (PerAddressDistributionRecord memory) {
  ...
  function getClaimableAmount(address beneficiary) public view virtual returns (uint256) {
    ...
    uint256 claimable = (record.total * getVestedFraction(beneficiary, block.timestamp, record.start, record.end, record.cliff)) /
      fractionDenominator;
    return
