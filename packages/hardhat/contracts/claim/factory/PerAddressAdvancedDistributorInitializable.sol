...
import {PerAddressDistributorInitializable, PerAddressDistributionRecord, IERC20} from "./PerAddressDistributorInitializable.sol";
 ...
 *   The PerAddressDistributionRecord for each beneficiary updated in the merkle root will be incorrect until a claim is executed.
 * - If the total changes, make sure to add or withdraw tokens being distributed to match the new total.
 */
abstract contract PerAddressAdvancedDistributorInitializable is
    Initializable,
    Ownable,
    Sweepable,
    ERC20Votes,
    PerAddressDistributorInitializable,
    IAdjustable,
    IVoting,
    FairQueueInitializable
{
    ...
    function _initializeDistributionRecord(address beneficiary, uint256 totalAmount, uint256 start, uint256 end, uint256 cliff) internal virtual override {
    ...
    function adjust(address beneficiary, int256 amount) external onlyOwner {
        PerAddressDistributionRecord memory perAddressDistributionRecord = records[beneficiary];
        require(perAddressDistributionRecord.initialized, "must initialize before adjusting");
        ...
        if (amount < 0) {
            ...
            require(perAddressDistributionRecord.total >= diff, "decrease greater than perAddressDistributionRecord total");
            total -= diff;
