    ...
    MerkleSetInitializable
{
    ...
    function initialize(
        ...
        __TrancheVesting_init(_token, _total, _uri, _maxDelayTime, uint160(uint256(_merkleRoot)), _owner);
    ...
    ) external validMerkleProof(keccak256(abi.encodePacked(index, beneficiary, amount, start, end, cliff)), merkleProof) {
        _initializeDistributionRecord(beneficiary, amount, start, end, cliff);
    }
