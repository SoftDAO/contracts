 ...
 */
struct PerAddressDistributionRecord {
...
interface IPerAddressDistributor {
  ...
  event InitializeDistributionRecord(address indexed beneficiary, uint256 total, uint256 start, uint256 end, uint256 cliff);
  ...
  function getDistributionRecord(
    address beneficiary
  ) external view returns (PerAddressDistributionRecord memory);
