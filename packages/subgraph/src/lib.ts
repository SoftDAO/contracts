...
import {
  ...
  AdvancedDistributor as AdvancedDistributorTemplate,
  CrosschainDistributor,
  TrancheVesting as TrancheVestingTemplate,
 ...
 */
export function getOrCreateNativePaymentMethod(
  oracle: Address,
  block: ethereum.Block
): PaymentMethod {
 ...
 */
export function getOrCreateTokenPaymentMethod(
  token: Address,
  decimals: i32,
  oracle: Address | null,
  block: ethereum.Block
  ...
  const paymentMethodId =
    token.toHexString() +
    "-" +
    (oracle ? oracle.toHexString() : "v1.x_priced_at_par") +
    "-" +
    decimals.toString();
  let paymentMethod = PaymentMethod.load(paymentMethodId);
  if (!paymentMethod) {
 ...
 */
export function getOrCreateDistributor(
  ...
  if (!distributor) {
    ...
    if (!tokenResult.reverted) {
      ...
      if (!decimalsResult.reverted)
        distributor.tokenDecimals = BigInt.fromI32(decimalsResult.value);
...
export function getAdvancedDistributor(address: Address): AdvancedDistributor {
  const advancedDistributor = AdvancedDistributor.load(
    `${address.toHexString()}-advanced`
  );
  if (!advancedDistributor)
    throw new Error(
      "advancedDistributor not found with id: " + address.toHexString()
    );
  return advancedDistributor;
}

export function getOrCreateAdvancedDistributor(
  ...
  const advancedDistributorContract = AdvancedDistributorContract.bind(
    distributorAddress
  );
  ...
  advancedDistributor.owner = ownerResult.reverted
    ? null
    : getOrCreateAccount(ownerResult.value, block).id;

  // get sweep recipient
  const sweepRecipient = advancedDistributorContract.try_getSweepRecipient();
  advancedDistributor.sweepRecipient = sweepRecipient.reverted
    ? null
    : getOrCreateAccount(sweepRecipient.value, block).id;

  // the address parameter here can be any value (not used at present)
  const voteFactorResult = advancedDistributorContract.try_getVoteFactor(
    distributorAddress
  );
  if (!voteFactorResult.reverted)
    advancedDistributor.voteFactor = voteFactorResult.value;
...
export function getOrCreateTrancheVesting(
  distributorId: string,
  block: ethereum.Block
): TrancheVesting | null {
  log.info("Trying to add tranche vesting info for distributor {}", [
    distributorId
  ]);
  const distributorAddress: Address = Address.fromString(distributorId);
  ...
  if (tranchesResult.reverted) {
    log.error(
      "Could not call distributor.getTranches() on distributor {}, is it mis-registered as tranche vesting?",
      [distributorId]
    );
    return null;
  }
  ...
  let trancheVesting: TrancheVesting | null = TrancheVesting.load(
    trancheVestingId
  );
  if (!trancheVesting) {
...
export function getOrCreatePriceTiers(
  distributorId: string,
  block: ethereum.Block
): PriceTier[] | null {
  log.info("Trying to add price tier vesting info to distributor {}", [
    distributorId
  ]);
  const distributorAddress: Address = Address.fromString(distributorId);
  ...
  if (priceTiersResult.reverted) {
    log.error(
      "Could not call distributor.getPriceTiers() on distributor {}, is it mis-registered as tranche vesting?",
      [distributorId]
    );
    return null;
  }
...
export function getOrCreateContinuousVesting(
  distributorId: string,
  block: ethereum.Block
): ContinuousVesting | null {
  log.info("Trying to add continuous vesting info to distributor {}", [
    distributorId
  ]);
  ...
  if (!continuousVesting) {
    ...
    if (result.reverted) {
      log.error(
        "Could not call distributor.getVestingConfig() on distributor {}, is it mis-registered as continuous vesting?",
        [distributorId]
      );
      return null;
    }
    ...
    log.info("**** CONTINUOUSVESTING {} {}", [
      continuousVesting.id,
      continuousVesting.createdAt.toString()
    ]);
  }

  return continuousVesting;
}
export function getOrCreateMerkleSet(
  distributorId: string,
  block: ethereum.Block
): MerkleSet | null {
  ...
  if (!merkleSet) {
    ...
    if (getMerkleRootResult.reverted) {
      log.error(
        "Could not call getMerkleRoot() on {}, is it an IMerkleSet contract?",
        [distributorId]
      );
      return null;
    }
...
export function getSaleImplementation(id: Address): SaleImplementation {
  ...
  if (!saleImplementation)
    throw new Error(
      "saleImplementation not found with id: " + id.toHexString()
    );
...
export function getDistributor(id: Address): Distributor {
  ...
  if (!distributor)
    throw new Error("distributor not found with id: " + id.toHexString());
  return distributor;
}
...
export function createDistributionRecord(
  ...
  const distributionRecordId =
    distributorAddress.toHexString() + "-" + beneficiaryAddress.toHexString();
  let distributionRecord = DistributionRecord.load(distributionRecordId);

  if (distributionRecord)
    throw new Error(
      "distribution record already exists with id: " + distributionRecordId
    );
...
export function getDistributionRecord(
  distributorAddress: Address,
  beneficiaryAddress: Address
): DistributionRecord {
  const distributionRecordId =
  ...
  if (!distributionRecord)
    throw new Error(
      "distributionRecord not found with id: " + distributionRecordId
    );
  return distributionRecord;
}

export function getOrCreateDistributionRecord(
  distributorAddress: Address,
  beneficiaryAddress: Address,
  block: ethereum.Block
): DistributionRecord {
  ...
  let distributionRecord = DistributionRecord.load(distributionRecordId);
  if (!distributionRecord) {
    return createDistributionRecord(
      distributorAddress,
      beneficiaryAddress,
      block
    );
...
export function getRegisteredAddress(
  ...
  if (!registeredAddress)
    throw new Error(
      "registeredAddress not found with id: " + registeredAddressId
    );
  return registeredAddress;
}
...
export function createClaim(
  ...
  const claimId =
    transaction.hash.toHexString() + "-" + transaction.index.toHexString();
...
export function createAdjustment(
  ...
  if (adjustment)
    throw new Error("adjustment already exists with id: " + adjustmentId);
