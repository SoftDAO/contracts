...
export function handleInitialize(event: Initialize): void {
  ...
  if (!sale) {
    log.error("no sale with id {} in tx {}", [
      saleId,
...
export function handleUpdate(event: Update): void {
  ...
  // save config
  sale.recipient = getOrCreateAccount(
    event.params.config.recipient,
    event.block
  ).id;
...
export function handleOwnershipTransferred(event: OwnershipTransferred): void {
  ...
  log.info("ownership transfered at address {} from {} to {}", [
...
export function handleBuy(event: Buy): void {
  ...
  if (isNative) {
    ...
    paymentMethod = getOrCreateTokenPaymentMethod(
      event.params.token,
      paymentTokenInfo.decimals,
      paymentTokenInfo.oracle,
      event.block
    );
  }
  ...
  if (event.params.tokenValue > BigInt.fromI32(0)) {
    ...
    price = event.params.baseCurrencyValue.times(
      BigInt.fromI32(10)
        .pow(paymentMethod.decimals as u8)
        .div(event.params.tokenValue)
    );
  } else {
  ...
  paymentMethod.purchaseTotal = paymentMethod.purchaseTotal.plus(
    purchase.spent
  );
  paymentMethod.purchaseCount = paymentMethod.purchaseCount.plus(
    BigInt.fromI32(1)
  );
  paymentMethod.save();
  ...
  implementation.purchaseCount = implementation.purchaseCount.plus(
    BigInt.fromI32(1)
  );
  implementation.save();
}
...
export function handleRegisterDistributor(event: RegisterDistributor): void {
  ...
  const distributor = getOrCreateDistributor(
